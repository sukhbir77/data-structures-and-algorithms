...
function pivotIndex(arr) {
  ...
  for (let j = 0; j < prefixSumArr.length; j++) {
    if (j == 0) {
      ...
      left = prefixSumArr[j - 1] - prefixSumArr[0] + arr[0];
    }
    if (j == prefixSumArr.length - 1) {
      right = 0;
    } else {
      right =
        prefixSumArr[prefixSumArr.length - 1] -
        prefixSumArr[j + 1] +
        arr[j + 1];
    }
    // console.log(`At index ${j} the left === ${left} and right === ${right}`);
    if (left === right) {
      return j
    }
  }
